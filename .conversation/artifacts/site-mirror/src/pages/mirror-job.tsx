...
export default function MirrorJobPage() {
  ...
  const resume = async () => {
    ...
    setOperationError(''); setResuming(true);
    try { const response = await fetch(`/api/mirror-jobs/${encodeURIComponent(job.id)}/resume`, { method: 'POST' }); if (!response.ok) throw new Error((await response.json()).error ?? 'Unable to resume mirror.'); await query.refetch(); } catch (error) { setOperationError(error instanceof Error ? error.message : 'Unable to resume mirror.'); } finally { setResuming(false); }
  };
