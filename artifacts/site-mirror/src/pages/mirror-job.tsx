...
export default function MirrorJobPage() {
  ...
  const resumeJob = useResumeMirrorJob();
  ...
  const canDownload = job.archiveAvailable && (job.status === 'completed' || job.status === 'completed_with_warnings');
  const cancel = () => {
    if (!window.confirm('Stop this mirror job? No additional requests will be made.')) return;
    setCancelError('');
    cancelJob.mutate({ id: job.id }, { onSuccess: (updated) => {
      queryClient.setQueryData(getGetMirrorJobQueryKey(job.id), updated);
      queryClient.invalidateQueries({ queryKey: getListMirrorJobsQueryKey({ limit: 50 }) });
    }, onError: () => setCancelError('The job could not be cancelled. Try again.') });
  };
  ...
  const retry = () => {
    ...
    retryJob.mutate({ id: job.id }, {
      ...
      onError: () => setOperationError('A new mirror run could not be started. Try again.'),
    });
  };
  const resume = () => {
    if (!window.confirm('Resume this mirror from its durable checkpoint? Saved results will be preserved.')) return;
    setOperationError('');
    resumeJob.mutate({ id: job.id }, {
      ...
      onError: () => setOperationError('Unable to resume mirror from its checkpoint. Try again.'),
    });
  };
  ...
  return (
    <div className="min-h-[100dvh] bg-[hsl(var(--background))]">
      ...
      <main className="mx-auto max-w-[1220px] px-5 py-8 md:px-10 md:py-11">
        ...
        {(cancelError || operationError) && <div data-testid="status-cancel-error" className="mt-4 rounded-xl border border-[hsl(var(--destructive)/.25)] bg-[hsl(var(--destructive)/.07)] px-4 py-3 text-xs font-semibold text-[hsl(var(--destructive))]">{cancelError || operationError}</div>}
        ...
        <section className="mt-3 grid gap-3 sm:grid-cols-4">
          <Stat icon={Gauge} label="Throughput" value={job.throughputBytesPerSecond ? `${formatBytes(job.throughputBytesPerSecond)}/s` : '—'} testId="text-throughput" />
          <Stat icon={ListChecks} label="Warnings" value={job.warningCount.toLocaleString()} detail={`${job.errorCount} errors`} testId="text-warning-count" />
          <Stat icon={Timer} label="ETA" value={job.estimatedRemainingMs == null ? '—' : `${Math.ceil(job.estimatedRemainingMs / 60000)} min`} testId="text-eta" />
          <Stat icon={FileArchive} label="Remaining" value={job.queueSummary.pending.toLocaleString()} detail="work items" testId="text-remaining" />
        </section>
        ...
        {job.events?.length > 0 && <section className="mt-7 rounded-[1.35rem] border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-[var(--shadow-sm)] md:p-7"><div className="mb-5 flex items-center gap-3"><ListChecks className="h-4 w-4 text-[hsl(var(--accent-foreground))]"/><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[hsl(var(--accent-foreground))]">Event log</p><h2 className="mt-1 text-lg font-extrabold">Recent crawl activity</h2></div></div><div className="max-h-72 space-y-2 overflow-auto">{job.events.slice().reverse().map((event, index) => <div key={`${event.at}-${index}`} className="flex gap-3 rounded-lg bg-[hsl(var(--muted)/.45)] px-3 py-2 text-xs"><span className="shrink-0 font-mono text-[9px] text-[hsl(var(--muted-foreground))]">{new Date(event.at).toLocaleTimeString()}</span><span className="font-semibold">{event.message}</span>{event.url && <span className="min-w-0 truncate font-mono text-[9px] text-[hsl(var(--muted-foreground))]">{event.url}</span>}</div>)}</div></section>}
