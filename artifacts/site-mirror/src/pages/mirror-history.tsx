...
import { getListMirrorJobsQueryKey, useListMirrorJobs, type MirrorJob } from '@workspace/api-client-react';
...
export default function MirrorHistory() {
  const query = useListMirrorJobs({ limit: 50 }, { query: { queryKey: getListMirrorJobsQueryKey({ limit: 50 }) } });
  const jobs = query.data?.jobs ?? [];
  const [search, setSearch] = useState('');
  const [status, setStatus] = useState('');
  const [warningsOnly, setWarningsOnly] = useState(false);
  const filteredJobs = useMemo(() => jobs.filter(job => (!search || job.url.toLowerCase().includes(search.toLowerCase())) && (!status || job.status === status) && (!warningsOnly || job.warningCount > 0)), [jobs, search, status, warningsOnly]);
  ...
  return (
    <div className="min-h-[100dvh] bg-[hsl(var(--background))]">
      ...
      <main className="mx-auto max-w-[1220px] px-5 py-8 md:px-10 md:py-11">
        ...
        <div className="mb-5 grid gap-2 md:grid-cols-[minmax(0,1fr)_180px_auto]"><input data-testid="input-history-search" aria-label="Search job history" value={search} onChange={e => setSearch(e.target.value)} placeholder="Filter by starting URL" className="h-10 rounded-xl border border-[hsl(var(--input))] bg-[hsl(var(--card))] px-3 text-xs outline-none"/><select data-testid="select-history-status" aria-label="Filter job status" value={status} onChange={e => setStatus(e.target.value)} className="h-10 rounded-xl border border-[hsl(var(--input))] bg-[hsl(var(--card))] px-3 text-xs"><option value="">All statuses</option><option value="running">Mirroring</option><option value="completed">Complete</option><option value="completed_with_warnings">Warnings</option><option value="failed">Failed</option><option value="cancelled">Cancelled</option></select><button data-testid="button-history-warnings" onClick={() => setWarningsOnly(v => !v)} className={`h-10 rounded-xl border px-3 text-xs font-bold ${warningsOnly ? 'border-[hsl(var(--accent-border))] bg-[hsl(var(--accent)/.18)]' : 'border-[hsl(var(--border))]'}`}>Warnings only</button></div>
        ...
        {filteredJobs.length === 0 && jobs.length > 0 && <div className="rounded-xl border border-dashed border-[hsl(var(--border))] p-8 text-center text-xs text-[hsl(var(--muted-foreground))]">No jobs match the current filters.</div>}
        ...
        <footer className="mt-10 flex flex-col gap-3 border-t border-[hsl(var(--border))] pt-5 text-[11px] text-[hsl(var(--muted-foreground))] sm:flex-row sm:items-center sm:justify-between"><span>Site Mirror keeps the crawl legible.</span><span className="font-mono tracking-[.1em]">{filteredJobs.length} JOB{filteredJobs.length === 1 ? '' : 'S'} SHOWN</span></footer>
      </main>
    </div>
  );
}
