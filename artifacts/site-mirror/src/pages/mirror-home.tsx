...
export default function MirrorHome() {
  ...
  const recentQuery = useListMirrorJobs({ limit: 1 }, { query: { queryKey: getListMirrorJobsQueryKey({ limit: 1 }) } });
  const createJob = useCreateMirrorJob();
  const latestJob = recentQuery.data?.jobs?.[0] ?? latestQuery.data;
  ...
  const applyPreset = (preset: 'quick' | 'docs' | 'assets' | 'cautious') => {
    const presets = {
      quick: { maxPages: 25, maxDepth: 2, requestDelayMs: 250, includeAssets: true, timeoutMs: 300000, maxTotalBytes: 104857600 },
      docs: { maxPages: 250, maxDepth: 6, requestDelayMs: 350, includeAssets: true, timeoutMs: 1800000, maxTotalBytes: 524288000 },
      assets: { maxPages: 100, maxDepth: 3, requestDelayMs: 400, includeAssets: true, timeoutMs: 1800000, maxTotalBytes: 1073741824 },
      cautious: { maxPages: 50, maxDepth: 2, requestDelayMs: 1000, includeAssets: false, timeoutMs: 900000, maxTotalBytes: 262144000 },
    }[preset];
    setForm(current => ({ ...current, ...presets }));
  };
      ...
      <main className="md:ml-[248px]">
        ...
        <div className="mx-auto max-w-[1260px] px-5 py-8 md:px-10 md:py-12">
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_350px]">
            <section className="mb-5 rounded-[1.35rem] border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 shadow-[var(--shadow-sm)]">
              <div className="flex items-center gap-3"><SlidersHorizontal className="h-4 w-4 text-[hsl(var(--accent-foreground))]"/><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[hsl(var(--accent-foreground))]">Start with a preset</p><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">Choose a scope, then adjust advanced controls if needed.</p></div></div>
              <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-4">{([['quick','Quick snapshot'],['docs','Documentation'],['assets','Asset-heavy'],['cautious','Cautious crawl']] as const).map(([key,label]) => <button key={key} type="button" onClick={() => applyPreset(key)} className="rounded-xl border border-[hsl(var(--border))] px-3 py-3 text-left text-xs font-bold transition-colors hover:border-[hsl(var(--accent-border))] hover:bg-[hsl(var(--muted))]">{label}<span className="mt-1 block text-[10px] font-normal text-[hsl(var(--muted-foreground))]">{key === 'quick' ? '25 pages · 100 MB' : key === 'docs' ? '250 pages · depth 6' : key === 'assets' ? '1 GB · assets on' : '50 pages · low rate'}</span></button>)}</div>
            </section>
            ...
            <aside className="space-y-6">
              <section className="rounded-[1.35rem] border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 shadow-[var(--shadow-sm)]">
                <div className="mb-4 flex items-center justify-between"><p className="font-mono text-[10px] uppercase tracking-[.18em] text-[hsl(var(--accent-foreground))]">02 / latest activity</p><Clock3 className="h-4 w-4 text-[hsl(var(--muted-foreground))]" /></div>
                 <RecentJob job={latestJob} isLoading={latestQuery.isLoading || recentQuery.isLoading} isError={Boolean(latestQuery.error || recentQuery.error)} />
              </section>
              <section className="rounded-[1.35rem] border border-[hsl(var(--border))] bg-[hsl(var(--secondary)/.52)] p-5">
                ...
                <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--background))] p-4">
                  <p className="font-mono text-[10px] uppercase tracking-[.16em] text-[hsl(var(--accent-foreground))]">Preflight</p>
                  <p className="mt-2 text-xs leading-5 text-[hsl(var(--muted-foreground))]">This run can discover up to <strong>{form.maxPages}</strong> pages at depth <strong>{form.maxDepth}</strong>, with a {form.requestDelayMs} ms request delay and a {formatBytes(form.maxTotalBytes)} total-byte ceiling. Robots: <strong>{form.respectRobotsTxt ? 'respected' : 'not enforced'}</strong>.</p>
                </div>
