  ...
  return (
    <div className="min-h-[100dvh] bg-[hsl(var(--background))]">
      ...
      <main className="mx-auto max-w-[1220px] px-5 py-8 md:px-10 md:py-11">
        ...
        <div className="mb-5 grid gap-2 md:grid-cols-[minmax(0,1fr)_180px_auto]"><input aria-label="Search job history" value={search} onChange={e => setSearch(e.target.value)} placeholder="Filter by starting URL" className="h-10 rounded-xl border border-[hsl(var(--input))] bg-[hsl(var(--card))] px-3 text-xs outline-none"/><select aria-label="Filter job status" value={status} onChange={e => setStatus(e.target.value)} className="h-10 rounded-xl border border-[hsl(var(--input))] bg-[hsl(var(--card))] px-3 text-xs"><option value="">All statuses</option><option value="running">Mirroring</option><option value="completed">Complete</option><option value="completed_with_warnings">Warnings</option><option value="failed">Failed</option><option value="cancelled">Cancelled</option></select><button onClick={() => setWarningsOnly(v => !v)} className={`h-10 rounded-xl border px-3 text-xs font-bold ${warningsOnly ? 'border-[hsl(var(--accent-border))] bg-[hsl(var(--accent)/.18)]' : 'border-[hsl(var(--border))]'}`}>Warnings only</button></div>
